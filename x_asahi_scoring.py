...
def _deep_merge_defaults(data: dict, defaults: dict) -> dict:
    """
    dataに存在しないキーをdefaultsから再帰的に補完する。
    値がdict同士の場合はネストした中身もマージする（部分的な欠損に対応）。
    値がdict以外（listやスカラー値）の場合はdata側の値をそのまま優先し、
    マージはしない（設定ファイルの意図的な上書きを尊重するため）。
    """
    for key, default_val in defaults.items():
        if key not in data:
            data[key] = default_val
        elif isinstance(default_val, dict) and isinstance(data.get(key), dict):
            data[key] = _deep_merge_defaults(data[key], default_val)
    return data
...
def load_asahi_config(force_reload: bool = False) -> dict:
    ...
    try:
        ...
        # 【バグ修正】旧実装はトップレベルキーが丸ごと無い場合しか補完しない
        # 浅いマージだった。"danger_score"キー自体は存在するが、その中の
        # "weights"サブキーだけが欠けている、というような部分的な欠損を
        # 補完できず KeyError を起こしていた。ネストした辞書も再帰的に
        # マージするよう修正する。
        data = _deep_merge_defaults(data, defaults)
        _CONFIG_CACHE = data
        return data
